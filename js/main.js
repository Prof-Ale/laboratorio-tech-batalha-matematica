...
function calcularPontos() {
    ...
    const final = (scoreA >= 70 || scoreB >= 70) ? 1.5 : 1; // desafio final +50%
...
function finalizarBatalha() {
    ...
    if (goTxt) goTxt.textContent = `🏆 ${vencedor} venceu a Batalha Matemática!`;
    if (goStats) {
        goStats.innerHTML = `
            <p>7ºA: <strong>${scoreA}</strong> pts &nbsp;|&nbsp; 7ºB: <strong>${scoreB}</strong> pts</p>
            <p>Rodadas: ${rodadaAtual - 1} &nbsp;|&nbsp; Acertos totais: ${G.acertos}</p>
        `;
    }
